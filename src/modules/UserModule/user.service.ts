...
export class UserService extends BaseService<User> {
	...
	async login(data: LoginDto) {
		try {
			const result = await this.userRepository
				...
				.leftJoinAndSelect(`${this.userRepository.metadata.name}.role`, 'roles')
				.getOne()
			if (!result) {
				throw new HttpException('User not found', HttpStatus.NOT_FOUND)
			}

			const checkPassword = Hash.compare(data.password, result.password)
			if (!checkPassword) {
	...
	async register(data: RegisterDto) {
		try {
			...
			const hash = Hash.genericHash(data.password)
			...
			const findCustomerRole = await this.roleService.getRoleByName(ROLE.USER)
